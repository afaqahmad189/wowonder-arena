...
if ($f == 'challanges') {
    if ($s == 'create' && Wo_CheckSession($hash_id) === true) {
        ...
        else if ($_POST['no_of_challange'] == '0') {
            $errors[] = $error_icon . $wo['lang']['please_choose_challanges'];
        }
        ...
        if (empty($errors)) {
            ...
            $product_data       = Wo_RegisterChallange($product_data_array);
            $challange_id         = 0;
            if (!$product_data) {
                $errors[] = $error_icon . $wo['lang']['please_check_details'];
            ...
            $challange_id = $product_data;
            $post_data  = array(
                'user_id' => Wo_Secure($wo['user']['user_id']),
                'product_id' => Wo_Secure($challange_id),
            ...
            if (count($_FILES['postPhotos']['name']) > 0) {
                for ($i = 0; $i < count($_FILES['postPhotos']['name']); $i++) {
                    ...
                    if (!empty($file)) {
                       $media_album = Wo_RegisterChallangeMedia($challange_id, $file['filename']);
                    }
                }
            }
            $data = array(
                'status' => 200,
                'href' => Wo_SeoLink('index.php?link1=post&id=' . $challange_id)
            );
        }
        header("Content-type: application/json");
        if (isset($errors)) {
