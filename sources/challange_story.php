...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    if (!empty($_GET['ref']) && is_numeric($_GET['ref']) && $_GET['ref'] > 0) {
        $wo['story'] = Wo_PostDataChallange($id, $placement, 'not_limited',Wo_Secure($_GET['ref']));
    }
    else{
       $wo['story'] = Wo_PostDataChallange($id, $placement, 50);
    }
    ...
    $wo['story']['page'] = 1;
    $wo['content']       = Wo_LoadPage('challange-content/content');
